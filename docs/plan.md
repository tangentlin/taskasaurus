...
### M6 — Hidden task filtering

- Parse `.vscode/tasks.json` (JSONC) to identify tasks with `"hide": true`.
- Build a set of hidden task labels per workspace folder.
- Filter out hidden tasks **before** running `buildHierarchy()`.
- Ensure hidden tasks do not contribute to group formation (e.g., if only one visible child remains after filtering, no group is created).
