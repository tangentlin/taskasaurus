...
import type { TaskKey, RootNode, RootLeafNode, ParentNode, ChildLeafNode, IconMap, NodeId } from './types';
...
export function buildHierarchy(tasks: vscode.Task[], iconMap: IconMap): RootNode[] {
  ...
  for (const info of taskInfos) {
    ...
    const belongsToGroup = (info.groupName !== undefined && validGroups.has(info.groupName))
  ...
  for (const [groupName, group] of groupMap) {
    ...
    if (group.runnableTask) {
      const runnableIndex = group.children.findIndex(
        c => c.label === group.runnableTask!.label && c.taskKey.source === group.runnableTask!.taskKey.source
      );
      if (runnableIndex > 0) {
